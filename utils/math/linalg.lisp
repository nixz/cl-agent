     ...
     (when (consp operands)
       (let ((acc  (copy-matrix (car operands))))
         (dolist (operand (cdr operands) acc)
           ...
           (map-into (matrix-data acc) ,op (matrix-data acc)
                     (matrix-data operand)))))))
  ...
  (let ((s (vec-length v)))
    (if (/= s 0)
        (make-instance 'matrix
                       :rows (matrix-rows v)
                       :cols (matrix-cols v)
                       :data (map '(simple-array single-float 1)
                                  #'(lambda (i)
                                      (/ i s))
                                  (matrix-data v)))
        (copy-matrix v))))
...
(defun cross (a b)
  (assert (is-3d-vector? a))
  (assert (is-3d-vector? b))
  (make-instance 'matrix
                 :rows 3
                 :cols 1
                 :data (make-array 3
