#|
(etf socket (make-instance 'inet-socket :type :stream :protocol :tcp))
(socket-bind socket (make-inet-address "127.0.0.1" ) 8030)
(socket-listen socket 4) ;... telnet
to the port...
(setf con (socket-accept socket))
(setf stream (socket-make-stream socket :input t :output t
:element-type 'base-char :buffering :none))
... and start happily using stream. What is distracting though is,
that the printed representation for that stream is:
#<FD-STREAM for "a constant string" {10E2ABD1}>
|#
...
(defun tcp-server (&key (interface (machine-instance)) (port 8000) (backlog 5))
  "Returns a socket server bound to PORT on INTERFACE.  Defaults
to (machine-instance):8000. The queue size BACKLOG defaults to 5
connections."
  (handler-case
...
(defun tcp-accept (server &key (timeout 30))
  "Waits up to TIMEOUT (defaults to 30) seconds for a client to
connect to SERVER.  A new socket is returned, which is bound to the
client connection.  If a timeout occured, nil is returned."
  (when server
...
(defun tcp-connect (server port &optional (timeout 5))
  "Returns a socket connected to SERVER:PORT.  If an error occurs, or
the attempt times out after TIMOUT (default 5) secons, nil is
returned."
    ...
    (handler-case
        (unless-timeout timeout
                        (let ((socket (make-instance 'sb-bsd-sockets:inet-socket
                          ...
                          socket))
      (host-not-fonund-error ()
        (format t "host ~A not found." server)
        (force-output)
        nil))))

;; so once tcp-connect has connected to a server, tcp-connect returns
;; a socket on the client ands tcp-accept on the server
...
(defun tcp-read-raw (socket &key (maxsize 65536) (timeout 5))
  "Reads one line from SOCKET, removes CRLF, and returns it.  The
buffer size is 65536 bytes, unless MAXSIZE is specified.  If no result
is received within TIMEOUT seconds (defaults to 5), nil is returned."
  (when socket
